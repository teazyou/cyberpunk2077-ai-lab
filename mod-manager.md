...
NOTE: CUSTOM MOD authored locally on user request (2026-07-05; auto-tag reworked to visible-sweep 2026-07-06; two-channel pickup + enemy LOS/any-range tag 2026-07-06; radius CRASH FIX → corpses-only + loot-iteration fix 2026-07-06) — not a Nexus download, nothing in downloaded/. Three independent scan-mode features (auto-pickup has two channels), each behind its own toggle(s) in the ScannerSuiteConfig block at the top of the .reds (edit literal + relaunch): EnableLootWhileScanning (default true), EnableAutoTagOnScan (default true), EnableAutoPickupOnScan (CURSOR channel, default true), EnableAutoPickupRadius (RADIUS channel, default true); secondary knobs AutoTagSweepRange (50.0 m non-enemy cap), AutoTagEnemyRange (500.0 m LOS-gated enemy cap), AutoTagSweepInterval (0.35 s), AutoPickupMaxDistance (cursor 20.0 m; was 12.0), AutoPickupRadius (10.0 m), AutoPickupPlaySound (true). Wraps 5 methods via @wrapMethod (each calls wrappedMethod exactly once — check here for collisions before installing scanner/HUD mods): HUDManager.OnScannerUIVisibleChanged (loot-while-scanning restore→delegate→suppress + arms the sweep loop on scanner-open when EnableAutoTagOnScan OR EnableAutoPickupRadius — ONE loop drives tag-sweep + radius-pickup) / OnQuickHackUIVisibleChanged / OnQuickHackUIKeepContextChanged (loot-while-scanning Path A: compensation keeps UIGameContext.Scanning OFF the UI context stack while preserving vanilla bookkeeping, state in @addField m_lwsScanningSuppressed) + HUDManager.OnLootDataChanged (debug probe only) + scannerDetailsGameController.OnScannedObjectChanged (SINGLE shared wrap dispatching auto-tag hover channel first, then auto-pickup cursor channel, gated on ActiveMode.FOCUS) (2026-07-06: a 6th wrap GameObject.OnGameAttached — radius-pickup registry maintenance — was REMOVED as the crash fix; nothing hooks the entity-attach lifecycle anymore, so the suite adds NO always-on per-entity hook and all custom work is scanner-scoped again). AUTO-TAG (reworked): while the scanner UI is up, a self-re-arming DelayCallback sweep loop (STSweepTickCallback + HUDManager.ST_ArmSweep/ST_SweepTick/ST_RunSweepOnce, 0.35 s cadence, armed-flag double-arm guard, self-terminates on scanner-close) runs TargetingSystem.GetTargetParts with TargetingSet.Frustum (through walls, NO LOS — occlusion sets Visible/ClearlyVisible deliberately unused) + camera-forward dot backstop (never behind player) + 50 m cap, deduped per tick (ST_RunSweepOnce); hover channel kept as complement (loot containers may lack TargetingComponents and be missed by the frustum query — settle via DebugProbeAutoTagSweep; in-file fallback if Frustum proves empty: TargetingSet.Complete + FOV cosine gate). ENEMY LOS/ANY-RANGE (2026-07-06): the alive-enemy category is split to a SEPARATE sibling pass ST_RunEnemySweepOnce on the SAME 0.35 s tick (its own GetTargetParts, TSF_Any(TSFMV.Obj_Puppet) puppet-only filter + AutoTagEnemyRange 500 m cap) that tags enemies ONLY in true line of sight (TargetingSystem.IsVisibleTarget — no through-wall) but at ANY range (ignores the 50 m cap); a not-yet-visible enemy is a transient skip spending nothing, tagging later once in LOS. All OTHER categories keep through-wall / 50 m / no-LOS. Classification is a SINGLE three-way ST_AutoTagCategory (None/Enemy/Other) on GameObject (evaluated once per target; thin ST_IsAutoTagWhitelisted wrapper kept): the main sweep tags Other only, the enemy sweep tags Enemy only. Categories BEFORE the seen-list: (1) ENEMY, alive safe-to-attack — CanBeTagged → IsHostile / IsCharacterCyberpsycho / DoNotTriggerPrevention-tag → crime-branch mirror of PreventionSystem.ShouldPreventionSystemReactToAttack (police/vendor/civilian/crowd/TriggerPrevention = never) → IsEnemy (this is the LOS/any-range category); (2) collectables — lootable corpses (puppet branch) + non-puppet loot objects gated TWICE: vanilla classification trio (IsContainer/IsShardContainer/IsItem — only loot classes override these; every Device inherits GameObject-base false, so doors/fridges/vending machines exit here) THEN non-empty TransactionSystem.GetItemList final gate (closes the empty-container hole; stash excluded); (3) access points un-breached (IsAccessPoint && !IsBreached; datamine-computers excluded); (4) security TURRETS only (IsTurret && !IsBroken — sole vanilla IsTurret override is SecurityTurret; disabled/unpowered still tagged). REMOVED 2026-07-06 — root cause of junk doors/fridges auto-tagging: the QUEST category ran FIRST, before any class gating — Device.IsQuest() reads persistent DeviceComponentPS.m_markAsQuest, which quests set on route/objective doors/appliances (Door/Fridge are InteractiveDevices) and often never clear, and GetAvailableClueIndex()≥0 fired on clue-flagged devices; whole QUEST category dropped (no IsQuest/clue tagging), CAMERAS also dropped per spec (IsSensor removed; turrets kept via exact-class IsTurret). Non-whitelisted targets spend NOTHING (no seen-list append). Once-per-entity semantics, all session-transient (never saved): auto-tag seen-list on FocusModeTaggingSystem (vanilla TagObject path; ResolveFocusClues removed from the auto-tag attempt 2026-07-06 — its TagLinkedCluekRequest cascade could tag non-whitelisted clue-group members such as quest doors; manual middle-click keeps the full vanilla cascade untouched; a manually untagged target is never re-tagged — its one attempt is spent); auto-pickup SHARED attempt ledger on PlayerPuppet (both channels — an entity attempted by one is not re-hammered by the other) with transient-vs-final semantics (alive NPC / locked / out-of-range / no-LOS / empty do NOT spend the attempt) + per-item filters (quest-flagged, iconic, HMG, nameless-non-shard skipped; locked containers and player stash never touched; replacer/braindance guarded; animated crates play their open animation on loot), all via the shared worker APS_TryAutoPickup on GameObject (which 2026-07-06 SNAPSHOTS item IDs/quantities into value arrays BEFORE transferring — TransferItem mutates the source inventory backing the live item list, so transferring while iterating it was a use-after-free; fixed). TWO PICKUP CHANNELS (2026-07-06): (a) CURSOR — hover via OnScannedObjectChanged, 20 m + LOS (raised from 12 m); (b) RADIUS — 360° auto-collect CORPSES/dead puppets within AutoPickupRadius (10 m) + LOS of the player, no cursor, run from the sweep tick (PlayerPuppet.APS_RunRadiusPickup — NO second timer, FOCUS-gated, same replacer/braindance guards; world containers/drops/shards are NOT covered by radius — the cursor channel collects those on hover). Radius ENUMERATION mechanism (2026-07-06 CRASH FIX): the original design was a CNML-style entity-attach registry — GameObject.OnGameAttached pushed loot-class entities into m_apsRadiusRegistry on PlayerPuppet. OnGameAttached runs on RED's MULTITHREADED entity-streaming (redDispatcher) workers, so its unsynchronized ArrayPush raced the allocator and corrupted the heap → hard EXC_BAD_ACCESS crashes faulting on worker threads inside the pool allocator (two identical crash reports; full evidence in wikis/modding/scanner-suite-crash-analysis.md). That hook + the m_apsRadiusRegistry field + APS_RadiusRegister were REMOVED. Radius pickup now enumerates on the GAME THREAD each tick via GameObject.GetEntitiesAroundObject(AutoPickupRadius, TSF_Any(Obj_Puppet)) — a vanilla TargetingSet.Complete 360° proximity query (the same call GetNPCsAroundObject and the settings menu use). Because it surfaces entities via TargetingComponent (only puppets/devices carry one), radius pickup is now CORPSES-ONLY by construction — world containers/drops/shards were never reachable this way and stay with the cursor channel. 10 m is the BINDING distance gate (the query distance-filters; the shared worker's 20 m cursor cap then always passes). No custom code runs off the game thread anymore, so the crash class is eliminated. Path A known side effect (by design): HUD elements vanilla hides during scanning (minimap, quest tracker, healthbar…) stay visible while the scanner is up; Path B fallback (Choice1 listener + TransferAllItems, no tooltip) documented in the plan, not deployed. Debug probe flags DebugProbeLootWhileScanning / DebugProbeAutoPickup / DebugProbeAutoTagSweep (keep false for play). PENDING in-game tests: loot-prompt-while-scanning decisive test T1 (+probe if it fails), the auto-pickup cursor blackboard probe (does UI_Scanner.ScannedObject carry plain loot objects?), the tag sweep probe (what does Frustum return — containers? through-wall? behind-player?), the enemy any-range/LOS tag check (far visible enemy tags; through-wall enemy does not), the radius-coverage probe (RADIUS: log line via DebugProbeAutoPickup — does GetEntitiesAroundObject surface nearby corpses within 10 m?), and a CRASH-REGRESSION check (play a streaming-heavy stretch — driving, fast-travel, district transitions — with the radius channel on; the removed attach hook must stay gone / no worker-thread crash). Stacking: no other installed mod wraps or replaces any of these methods (grep-verified across all deployed .reds; the simple-untrack-quest/better-fast-travel @replaceMethod collision is WorldMapMenuGameController — unrelated). Plans + research dossiers archived at wikis/modding/ (rework spec: scanner-suite-refinements.md).
DESC: Scan-mode trio — keeps the vanilla loot prompt usable while scanning; auto-tags everything whitelisted while the scanner is up (through-wall frustum sweep within 50 m for loot-bearing collectables / un-breached access points / working turrets, PLUS a separate LOS-gated any-range sweep for no-NCPD-heat enemies; quest elements and cameras removed 2026-07-06; once per entity via the vanilla tag path); and auto-collects lootables via two channels — cursor-hover within 20 m + LOS and a 360° radius corpse-vacuum within 10 m + LOS of the player (corpses/dead puppets only — world containers/drops left to the cursor channel; quest/iconic/HMG filtered, shared once-per-entity ledger) — each feature independently toggleable in the .reds.
