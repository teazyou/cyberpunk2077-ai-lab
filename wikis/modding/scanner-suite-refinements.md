...
## 2026-07-06 — two-channel pickup + enemy LOS

Three changes shipped to `ScannerSuite.reds` (compiles clean, zero custom-scanner-suite warnings; the 4 pre-existing warnings from simple-untrack-quest / better-fast-travel / disassemble-loot / drink-at-the-counter are unrelated): (1) cursor pickup range 12 → 20 m; (2) a NEW 360° radius pickup channel (10 m); (3) the auto-tag ENEMY category made LOS-gated + any-range.

### Radius enumeration mechanism — CNML-precedent entity-attach registry (DECISIVE decompile evidence)

**Chosen:** wrap `GameObject.OnGameAttached` to register loot-class entities into a session array (`m_apsRadiusRegistry` on `PlayerPuppet`), then distance-filter that registry each sweep tick. This is exactly CNML's (Nexus 16040) mechanism.

**Why not a targeting/frustum query (the tempting simple option) — VERIFIED it structurally cannot see loot:** `TargetingSystem.GetTargetParts` (orphans.swift:22383) returns `TS_TargetPartInfo`s, and the ONLY way to get an entity out is `TS_TargetPartInfo.GetComponent(part) -> wref<TargetingComponent>` then `.GetEntity()` (orphans.swift:28846; the pattern in `GameObject.GetEntitiesAroundObject` gameObject.swift:686 and `HighlightEffector.RevealAllObjects` highlightEffector.swift:64). So an entity with **no `TargetingComponent` is unreturnable by any `TargetingSet`** (Frustum/Complete/Visible/…). Who requests that component (grep of `RequestComponent(... gameTargetingComponent ...)`):
- `ScriptedPuppet` (scriptedPuppet.swift:430-433) — so CORPSES/NPCs *are* returnable.
- `deviceBase` (deviceBase.swift:348), `SensorDevice` (sensorDevice.swift:33), `SecurityTurret` (securityTurret.swift:51) — devices/sensors/turrets returnable.
- `gameLootContainerBase` (lootContainers.swift:409+), `gameLootObject`/`gameItemDropObject` (inventoryComponent.swift:83/162), `ShardCaseContainer` — **request NONE** (zero TargetingComponent hits in lootContainers.swift / inventoryComponent.swift). So containers/dropped items/shard cases are invisible to targeting queries. This upgrades the prior dossier's "SPECULATED-LIKELY" gap to VERIFIED.

**Why not `GameInstance.GetEntityList()`** (orphans.swift:11551): it returns the ENTIRE world entity list; its sole vanilla caller is `PlayerDevelopmentSystem.ScaleNPCsToPlayerLevel` (playerDevelopmentSystem.swift:2522), a one-shot level-scaler — not a per-tick poll. Calling it at 0.35 s would allocate + iterate every entity in the world each tick — the exact per-tick cost the spec warns against.

**Registration gate (mostly CLASS-based, state-independent so loot-init timing can't drop a container):** `!IsPlayer() && (IsPuppet() || IsDefined(this as gameLootContainerBase) || IsDefined(this as gameLootObject) || IsItem())`. Verified hierarchy makes the two casts complete: `gameContainerObjectBase extends gameLootContainerBase` (lootContainers.swift:693) and `ContainerObjectSingleItem extends gameContainerObjectBase` (orphans.swift:35180) → crates, animated lockers/fridges (`LootContainerObjectAnimatedByTransform`, :837) and shard cases (`ShardCaseContainer extends ContainerObjectSingleItem`) are ALL `gameLootContainerBase`; `gameItemDropObject extends gameLootObject` covers dropped items; `IsItem()` = `(this as ItemObject) != null` (gameObject.swift:1395, class-based). Empty-at-attach containers still register (cast is class not state) — closing the `IsEmpty()`-before-lazy-init hole for the radius channel.

**Lifecycle wiring:** the sweep loop now arms when `EnableAutoTagOnScan() OR EnableAutoPickupRadius()` (was tag-only) in `OnScannerUIVisibleChanged`, and `ST_SweepTick` early-outs only when BOTH are off (was tag-only) — otherwise radius pickup died whenever auto-tag was toggled off. `wrappedMethod` still called exactly once on every path. Inside FOCUS the tick runs `ST_RunSweepOnce` + `ST_RunEnemySweepOnce` (if tag on) and `player.APS_RunRadiusPickup(game)` (if radius on) — ONE `DelayCallback` loop, no second timer. Registry compacted (null wrefs dropped) each pass, so no `OnDetach` wrap (avoids an O(n) `ArrayRemove` on every global detach).

**Shared-ledger / distance-gate interplay:** both channels share `m_apsAttempted` (`APS_AlreadyAttempted`/`APS_MarkAttempted`) + the shared worker `APS_TryAutoPickup`, so an entity attempted by radius is not re-hammered by cursor and vice-versa; transient refusals (alive/locked/no-LOS/empty) leave it re-eligible for either. The worker enforces `AutoPickupMaxDistance` (now 20 m) internally; the radius pass pre-filters to `Vector4.Distance <= AutoPickupRadius()` (10 m) BEFORE calling it, so 10 m is the binding gate for radius and the 20 m cursor cap can never leak in. LOS (`IsVisibleTarget`) stays mandatory in the worker for both channels.

### Enemy any-range tag — TWO-query approach (chosen over one uncapped query)

The main 50 m frustum sweep now tags the OTHER categories only; enemies are handled by a SEPARATE sibling pass `ST_RunEnemySweepOnce` on the same tick: a second `GetTargetParts` with `searchFilter = TSF_Any(TSFMV.Obj_Puppet)` (puppets only — humans/drones/mechs; `TSFMV.Obj_Puppet = 1`, orphans.swift:3173, same enum the file already uses via `TSFMV.Obj_Player`) and `maxDistance = AutoTagEnemyRange()` (500 m, `filterObjectByDistance = true`). Per enemy the gates run cheapest-first: camera-forward dot → `ST_AutoTagCategory() == Enemy` → `!AutoTagAlreadySeen` → `IsVisibleTarget` (occlusion raycast LAST, only for unseen in-front enemies). "Not visible" appends nothing (transient) so the enemy tags on a later tick once in LOS — identical to the collectables empty-list rule.

**Why two queries not one:** option (b), a single uncapped (`filterObjectByDistance=false`) all-object frustum query, returns EVERY device + puppet in the frustum to infinity and forces a script-side distance calc on all of them. Two targeted queries keep the main sweep bounded at 50 m (unchanged cost) and add only a puppet-filtered pass whose per-tick work is proportional to NPCs-in-view, not all-entities-to-infinity. Chose a large-but-bounded 500 m cap over truly unbounded per the spec's cost guidance (500 m covers every realistic scanner sightline; enemies past it tag once they close in).

**Classification refactor:** `ST_IsAutoTagWhitelisted()` (Bool) → `ST_AutoTagCategory()` (enum `STAutoTagCategory{None,Enemy,Other}`), a SINGLE evaluation per target that the two sweeps branch on without double-classifying; a thin `ST_IsAutoTagWhitelisted` wrapper is kept. Lootable corpses classify as **Other** (checked before the enemy chain) so they keep through-wall/50 m behavior; only ALIVE attackable puppets are **Enemy**. The hover channel also applies the enemy-LOS gate (one cheap `IsVisibleTarget` line) so a through-wall hovered enemy isn't tagged until visible — kept symmetric with the sweep rather than left asymmetric.

### Verified signatures (this pass)

`TargetSearchQuery` fields (orphans.swift:28819) unchanged; `TargetingSet{Visible=0,ClearlyVisible=1,Frustum=2,Complete=3,None=4}` (orphans.swift:3194); `TSF_Any`/`TSF_Not`/`TSF_And`/`TSF_All` (orphans.swift:28836-28842); `TSFMV.Obj_Puppet=1` (orphans.swift:3171); `GameObject.OnGameAttached()`/`OnDetach()` `protected cb -> Bool` (gameObject.swift:356/366); `GameObject.IsPlayer()` (1323), `IsPuppet()` (1319), `IsItem()` protected (1395); global `GetPlayer(GameInstance) -> ref<PlayerPuppet>` (player.swift:2); `HUDManager.GetPlayer() -> ref<GameObject>` (hudManager.swift:1363); `IsReplacer` (gameObject.swift:1331), `GetHudManager` (2476), `IsBraindanceActive` (hudManager.swift:438); `IsVisibleTarget` (orphans.swift:22453). Enum comparison via `Equals`/`NotEquals` on a custom enum is valid (CNML precedent: `Equals(x, cnmlSeenStatus.Seen)`).

### Per-tick cost note

Per 0.35 s tick while scanning + FOCUS: (1) main sweep — one 50 m `GetTargetParts` + cheap cached-bool classify per return (unchanged); (2) enemy sweep — one puppet-only 500 m `GetTargetParts` + one `IsVisibleTarget` raycast per unseen in-front enemy (few in view; LOS is the only non-trivial op and short-circuits after category); (3) radius pass — a walk of `m_apsRadiusRegistry` (lean: loot classes + puppets only, not every world object; compacted of nulls each pass) with a cheap `Vector4.Distance` reject per entry, and the expensive `GetItemList`/`IsVisibleTarget`/transfer only for entities within 10 m. Always-on cost outside scanning: `OnGameAttached` fires per entity stream-in and does a toggle check + up-to-4 cached-bool/cast predicates before (rarely) registering — comparable to CNML, which registers every object unconditionally. No second timer added; the whole thing rides the existing sweep `DelayCallback`.

---

## 2026-07-06 — radius pickup aborted; cursor pickup re-gated to LOS

Follow-up to the "two-channel pickup + enemy LOS" change above. Two edits shipped to `ScannerSuite.reds` (compiles clean, zero custom-scanner-suite warnings):

- **RADIUS channel removed entirely (ABORTED).** The 360° corpse-vacuum channel added above (`EnableAutoPickupRadius`/`AutoPickupRadius` + `APS_RunRadiusPickup` game-thread `GetEntitiesAroundObject` enumeration) is deleted, along with the per-tick radius pass and cost item (3) above. It only ever worked on corpses — world containers/drops/shards carry no `TargetingComponent`, so no crash-safe proximity query enumerates them through walls (the VERIFIED gap this dossier documented) — so it added nothing the cursor channel doesn't already cover. The suite is a single cursor pickup channel again. The `OnGameAttached` registry this channel originally used was already removed as the heap-corruption crash fix; nothing hooks the entity-attach lifecycle now (do NOT re-add it).
- **CURSOR pickup LOS RE-ENABLED.** Reverts the interim "no-LOS through-wall" cursor policy back to Refinement 2's standing verdict (**KEEP `IsVisibleTarget`**): `GetLookAtObject` is set to the LOS form `(this, true, true)` (vanilla `focusModeTagging.swift:205` precedent), and `APS_TryAutoPickup` re-checks `TargetingSystem.IsVisibleTarget(player, target)` after the range gate. Only loot in actual line of sight within `AutoPickupMaxDistance` (unchanged, 40.0 m) is auto-picked; occluded loot is a transient refusal. Auto-TAG keeps its deliberate through-wall behavior (information through walls = the scanner fantasy); auto-PICKUP keeps LOS (physically moving items through walls is not) — the Refinement 2 asymmetry is restored.

## 2026-07-06 — auto-tag: enemies + access-points/turrets dropped, Epic quality floor added

Auto-tag scope tightened to loot only (no enemy tagging). All changes are in the single classifier `GameObject.ST_AutoTagCategory`, so the sweep and hover channels inherit them; compiles clean, zero custom-scanner-suite warnings; `@wrapMethod` count unchanged (still 5); no attach/streaming hook added.

- **ENEMY category removed.** The alive safe-to-attack puppet branch and the whole separate LOS/any-range enemy sweep (`ST_RunEnemySweepOnce` + its `ST_SweepTick` call + the `AutoTagEnemyRange` config) are deleted. The enum is now `STAutoTagCategory { None, Other }`. The puppet branch keeps ONLY the lootable-corpse → `Other` case.
- **ACCESS-POINT + TURRET categories removed.** The `IsAccessPoint` and `IsTurret` branches are gone; the only `Other` outcomes are (a) a lootable corpse and (b) a loot-bearing world object (container/shard/item).
- **Epic quality floor added.** Both collectable cases now require MAX item quality ≥ `AutoTagQualityFloor` (default `gamedataQuality.Epic`, Tier 4; user-editable). New helpers `ST_LootMeetsQualityFloor` + `ST_QualityTier` — the tier compare is an EXPLICIT switch, not `EnumInt` (non-monotonic ints: Epic=2, Rare=9, Invalid=14) and not `RPGManager.ItemQualityEnumToValue` (drops EpicPlus/LegendaryPlus/Iconic to 0); EpicPlus/LegendaryPlus/LegendaryPlusPlus/Iconic count as ≥ Epic, Rare/Uncommon do not.
- **Side effect (intended):** shards and below-Epic junk corpses/containers no longer auto-tag. Below-floor/empty stays transient (no seen-list append) → re-checked when a streamed container exposes loot, or lower the floor to `gamedataQuality.Rare` for Tier 3+.

## 2026-07-07 — auto-pickup: LOS exemption for corpses + floor items ("some corpses never loot" fix)

Diagnosis (sub-agent, static read of `ScannerSuite.reds`): the F2 surroundings channel + all pickup paths share worker `APS_TryAutoPickup`; every skip in it is a TRANSIENT refusal (no `m_apsAttempted` spend), so a condition that never clears = a body never looted. ROOT CAUSE = the LOS gate `TargetingSystem.IsVisibleTarget(player, this)` (worker, ~:950). It probes a `ScriptedPuppet`'s body-part `TargetingComponent` points (scriptedPuppet.swift:430-433); a ragdolled corpse's points clip into floor/cover/adjacent bodies, so a plainly-visible body FAILS the query and is re-refused every ~1 s forever. Bodies in the open pass → looted. Exactly "some corpses, not others." No faction/quality filter is involved (ruled out: no faction check anywhere in the worker; Epic floor is auto-TAG-only).

- **FIX — LOS exemption.** After the range gate, compute `losExempt = IsDefined(puppet) || this.IsItem()` and skip `IsVisibleTarget` when true. So **corpses (dead/incap puppets)** and **floor items** (weapon drops redirect via `APS_ResolveLootTarget` to their `gameItemDropObject`, `IsItem() == true`) are gated by RANGE ALONE (`AutoPickupMaxDistance`, unchanged). **Containers + shard cases KEEP LOS** — physically pulling a crate's contents through a wall stays disallowed; the Refinement-2 asymmetry (info through walls yes, moving items through walls no) is preserved for containers only. Bodies/floor-drops are the exception because their occlusion query is unreliable, not because through-wall grab is desired.
- **Scope note:** exemption covers ALL floor drops (`IsItem()`), not weapons only — REDscript can't cleanly isolate weapon drops at the drop object, and ammo/grenades/consumables on the floor are wanted too. If weapons-strict is ever needed it'd require a per-item-type re-check inside the transfer loop, not a target-level gate.
- Single-block edit in `APS_TryAutoPickup`; no new config flag, no new `@wrapMethod`, no attach/streaming hook. Takes effect on next `launch_modded.sh` compile (not compiled in-session — avoids the transient backup-corrupted dialog mid-workflow).

## 2026-07-07 — auto-pickup: loop-death + ledger-lockout fixes ("stops after a few runs" / "some corpses still skipped")

Second-pass audit (sub-agent, static) after the LOS exemption above still left corpses skipped + a new "loop gets stuck after a few runs" symptom. Two independent bug classes found; four fixes applied to `ScannerSuite.reds`, two skipped by design. Iteration bound, interval accumulator, scan/FOCUS toggle, fresh save/load all audited CLEAN (not causes). Not compiled in-session.

**Loop death — why F2 stopped after N ticks:**
- **1A `APS_LootLoopTick` re-arm was the LAST statement (:751 now)** — successor was scheduled only AFTER `APS_RunCursorPickup` / `APS_RunSurroundingsPickup` / the worker ran. Any REDscript runtime fault in that work (e.g. an unguarded stale/null `itemData` wref from a just-killed, still-mutating inventory) aborts the callback frame before the re-arm → loop silently dies permanently. Fits "runs a few times then stops the moment it meets one bad body." **FIX: arm the successor FIRST**, right after the both-off stop and before the replacer/braindance check + all pickup work. Removed the trailing arm and the braindance-branch arm (would double-arm). Now exactly one re-arm/tick, fault-proof; both-off still permanently stops (`m_apsLoopArmed=false; return;` with no arm); replacer/braindance still returns-but-loops.
- **item deref guard** — PASS-1 loop (`:1004`) now `if !IsDefined(itemData) { continue; }` before any `itemData.*` read. Closes the fault surface behind 1A.

**Ledger lockout — why specific corpses stayed skipped even with the loop alive:**
- **2A worker over-spent the attempt (:1026-1049)** — old tail did unconditional `return true` whenever `GetItemList` was non-empty, so a body whose list at first contact held ONLY nameless placeholders (or whose real loot/equipped-weapon-on-death hadn't resolved yet) got `APS_MarkAttempted`-ed FINAL → when its real loot populated a beat later it was already `AlreadyAttempted` → never retried. NPCs vary in resolve timing → "some corpses, not others." **FIX: `if !lootedAny { return false; }`** — a non-empty list that transferred nothing is now TRANSIENT (stays eligible, cheap re-check next pass); only a pass that actually transferred ≥1 item spends the attempt. Genuinely-empty-list transient (`:985`) and the container/stash/non-lootable FINAL returns unchanged. Cost: a body of purely-nameless placeholders re-reads `GetItemList` each pass forever (rare, cheap).
- **2B ledger unbounded + EntityID reuse (:646-657)** — `m_apsAttempted` was never cleared; engine recycles `EntityID`s after despawn, so late-session a fresh corpse could inherit a retired ID → false "already attempted." **FIX: FIFO cap** — `ArrayErase(m_apsAttempted, 0)` when size >4096, so recycled IDs age out and the array can't grow unbounded. No streaming/detach hook (heap-corruption history — forbidden).

**SKIPPED (by design):**
- **1B `m_apsLoopArmed` stale-true wedge** — the guard genuinely prevents a double-loop when `OnGameAttached` fires twice with a callback pending; bypassing it (always re-arm) risks compounding loops. Wedge scenario (same-`PlayerPuppet`-instance re-attach after a delay-queue flush) is unconfirmed and 1A already removes the dominant death cause. Left as-is.
- **2C separate `gameLootBag`/`gameItemDropObject` stays LOS-gated** — exempting drop/bag classes from `IsVisibleTarget` would enable through-wall grabbing of any dropped loot within 50 m (forbidden). Corpse inventory lives on the puppet (already exempt via `IsDefined(puppet)`), so the corpse case needs no bag exemption.

## 2026-07-07 — auto-tag quality floor lowered Epic → Common (tag every tier)

User wanted the previously-excluded low tiers (white/Common + blue/Rare) tagged too. A monotonic floor can't express "white and blue but not green," so — per user's pick — lowered the floor to the LOWEST tier = tag EVERYTHING with loot. Config-only change (+ comment accuracy), auto-pickup/loop/ledger untouched.

- `AutoTagQualityFloor()` (:133): `gamedataQuality.Epic` → `gamedataQuality.Common`. Now every real quality tags (white/green/blue/purple/orange/iconic). Raise back toward `Epic` to restrict.
- Machinery verified unchanged/correct at the new value: `ST_QualityTier` (:403-424) has `case gamedataQuality.Common/CommonPlus: return 1` — rank **1 is the minimum real rank** (Uncommon=2 … Iconic=6; +/++ variants fold to base; only Random/Count/Invalid/unresolved `default` returns 0, below Common). `ST_LootMeetsQualityFloor` (:434-458) returns `bestTier >= floorTier` with `floorTier = tier(Common) = 1`, so any object with ≥1 real-quality item passes; a list with no resolvable quality stays `bestTier=0 >= 1` false → sensibly not tagged (no crash). No `ST_QualityTier` edit needed.
- Header Feature-2 block + two inline doc-comments updated Epic→Common for accuracy. (The dated 2026-07-06 "floored at Epic" REMOVED note left as historical record.)

**Compile outcome (first launch attempt, fixed):** `continue` is NOT a REDscript keyword — `[UNRESOLVED_REF] unresolved reference 'continue'` at :1005 failed the whole `custom-scanner-suite` compile. Fixed by inverting the guard to wrap the loop body in `if IsDefined(itemData) { … }` (no `continue` needed). `ArrayErase(arr, index)` compiled clean — kept. Lesson for future edits: REDscript has no `continue`/`break`-to-skip; use an `if`-wrapper. (The only other log lines were pre-existing WARNs in OTHER mods — `disassemble-loot`, `drink-at-the-counter` — unrelated.)

## 2026-07-12 — 360° radius auto-pickup re-activated (10 m)

`ScannerSuiteConfig.EnableConstantAutoLoot()` flipped to **true** (was false). Config + comments only; compile clean.

Effect: the always-on loot loop (`APS_LootLoopTick`) now runs BOTH channels — cursor look-at every 0.35 s, plus the 360° `GetEntityList` radius pass every 1.0 s (`ConstantAutoLootInterval`) — both bounded by the shared `AutoPickupMaxDistance() = 10.0`. Radius keeps the worker LOS gate for containers/shard cases; corpses + floor items are LOS-exempt as before. Ledger `m_apsAttempted` is shared, so a channel can't re-loot what the other took.

Distinct from the 2026-07-06 "radius aborted" note above: that abort applied to the *TargetingComponent* enumeration (blind to loot classes). The live channel enumerates the raw world entity list on the game thread and sees every loot class. No entity-attach/streaming hook (crash-forbidden) is used.

## 2026-07-12 — cursor pickup 10 m + auto-tag no longer drops quality-less loot

Two user-requested changes to `ScannerSuite.reds`; validated with serial `scc -compile` (clean, no custom-scanner-suite diagnostics).

- **Auto-pickup range (mouseover):** `AutoPickupMaxDistance()` 6.0 → **10.0**. Shared by the cursor channel (its only reach limit — cursor ignores LOS) and the F2 surroundings pre-filter (off by default). Config + comments only.
- **Auto-tag: materials / broken weapons now tag.** Enemy-dropped quality-less loot has no Quality stat → `RPGManager.GetItemDataQuality` yields `Random`/`Invalid` → `ST_QualityTier` `default` = **tier 0** → the old `bestTier >= floorTier` tail rejected it at the Common floor (tier 1). This is precisely the failure the 2026-07-07 entry below predicted and waved through ("a list with no resolvable quality stays `bestTier=0 >= 1` false"). `ST_LootMeetsQualityFloor` now tracks a separate `hasLoot` flag: no loot → `false` (transient, as before); `floorTier <= 1` → `true` for any loot-bearing target regardless of tier (tag-everything, the Common-floor intent); the tier compare survives only for raised floors. Vanilla `CanBeTagged()` was ruled out as the cause (defaults true, only `ScriptedPuppet` overrides — `gameObject.script:1997`).
